...
支持 **「上架月份」** 过滤：可指定一个 `YYYY-MM`（如 `2025-11`），仅扫描**该月在币安上架的合约**（上架时间取自 `listings` 表，以合约首次 K 线时间为准，缺失时回退到公告日期）；留空则不限制。指定月份后候选集合变小，扫描更快。结果表会额外展示每只合约的「上架」月份。

> 上架时间以服务器时区（Docker 默认为 UTC）计算月份，跨月边缘（如月底最后 1 小时上架）可能在 UTC 与本地时区有 ±1 月差异，属预期行为。
